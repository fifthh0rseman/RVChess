...
def loadModel(lang):
    modelle = Model('./data/vosk-model-small-ru-0.22')
    if lang == 'ru-small':
        modelle = Model('./data/vosk-model-small-ru-0.22')
    elif lang == 'en-small':
        modelle = None
...
def startExperiment(mode, stream, rec):
    if mode == str(1):
        ...
        print("Выбран режим записи в файл с быстрым произношением.")
        file = open('../tilesFast.txt', 'a')
    elif mode == str(7):
        print("Выбран режим записи в файл с медленным произношением.")
        file = open('../tilesSlow.txt', 'a')
    else:
